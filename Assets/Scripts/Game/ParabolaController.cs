...
public class ParabolaController : MonoBehaviour
{
    ...
    Transform pointA, pointB, pointC;

    void OnDrawGizmos()
    {
        if (ParabolaRoot.transform == null)
            return;
    ...
    public void ThrowGrenade(Transform a, Transform c, float speed, float height, GameObject _parabolaRoot)
    {
        Speed = speed;

        ParabolaRoot = _parabolaRoot;
        pointA = ParabolaRoot.transform.GetChild(2);
        pointB = ParabolaRoot.transform.GetChild(1);
        pointC = ParabolaRoot.transform.GetChild(0);
        pointA.position = a.position;
        pointC.position = c.position;

        pointB.position = (c.position + a.position) / 2;
        Vector3 temp = pointB.position;
        temp.y = height; // grenade height
